...
- **Seventh companion — Rattle Bones.** Added 2026-05-26 as a third locked teaser slot. A skeletal keeper — bone-clatter from the crypt. Ships **locked** with only her idle portrait wired up (same teaser treatment as Luna / Nocturna). Full expression bank, dialogue, and unlock condition are deferred. Source art: `Quest-Failed assets/Companions/Rattle Bones`, baked to `assets/npc-rattlebones/`.
- **Eighth companion — Necroknight.** Added 2026-05-26 as a fourth locked teaser slot. Armored undead warrior — "Sworn to no king, served by every restless dead." Ships **locked** with only his idle portrait wired up. Accent colour: spectral phosphor green (`#4dff7a`) so his hover/select halo reads as ghostfire on the recruit card and his future in-game NPC bubble border carries the same green. Full expression bank, dialogue, and unlock condition are deferred. Source art: `Quest-Failed assets/Companions/The Necroknight/` (the source folder still has "The" — the in-game display name dropped it 2026-05-26 per user request), baked to `assets/npc-necroknight/`.
- **Ninth companion — Spectra.** Added 2026-05-26 as a fifth locked teaser slot. A wraith — "a wraith's whisper at the edge of every shadow." Ships **locked** with only her idle portrait wired up (same treatment as the other teasers). Accent colour: default until specified. Full expression bank, dialogue, and unlock condition are deferred. Source art: `Quest-Failed assets/Companions/Spectra`, baked to `assets/npc-spectra/`.
- **Unlock conditions — DEFERRED.** Specific unlock terms (boss-level requirement / specific event / achievement / etc.) for Nocturna, Luna, Rattle Bones, Necroknight, Spectra, and any future companions are **not yet decided**. The system is built so the unlock model can be plugged in later as a data edit + a single `unlockCompanion()` call at the right trigger point.
