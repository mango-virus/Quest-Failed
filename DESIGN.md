...
7. **Reclaimed by defeat (2026-05-28):** the mark isn't permanent. If the Solo Leveling event **recurs later in the same run** and this time the boss **kills Jinwoo**, it breaks free of his claim — the shadow-flame + blue tint **drop at the next night**, so the boss looks normal again from the following build phase on. (The boss keeps the flame through the rest of that day for the death cutscene; the clear lands on the night boundary. Persisted, so a save/load between the kill and the night still clears it.)
