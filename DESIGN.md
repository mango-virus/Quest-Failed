...
Rooms connect to each other directly through their doorways — when you place a room near another, it auto-aligns so a doorway on one room sits next to a facing doorway on the other. There are no auto-routed corridor segments drawn between rooms. **Corridors are a placeable room type** (see room roster) used to extend reach between rooms; adventurers walk through them like any other room. (deviation noted: original prototype had auto-routed drawable corridor segments — replaced with doorway-snap placement plus a placeable Corridor room so every cell of the dungeon is meaningful gameplay space.) **(deviation 2026-06-24, LOCKED: rooms now connect only when placed exactly ONE TILE APART, not wall-to-wall; that gap becomes a 2-wide black "connector" door the rooms open onto and characters walk under. Touching placement is forbidden. Full spec + acceptance checklist in `ROOM_CONNECTIONS.md`.)**
