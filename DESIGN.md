...
- **Unlock conditions — partially DEFERRED.** As of 2026-05-26 the wired unlock achievements are: Zul'Gath ← `hoard_lord` (10,000 gold in a run), Rattle Bones ← `curtain_call` (100 trap kills in a run). Nocturna, Luna, Necroknight, Spectra, and any future companions still have **deferred** unlock terms — the system is built so each can be plugged in later as a data edit + a single `unlockCompanion()` call (or reward-tagged achievement) at the right trigger point.
