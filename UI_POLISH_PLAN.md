...
- **Design (built):** rebuilt `WelcomeIntroOverlay` as a paced **3-step** intro in the crypt `Overlay` shell, with real imagery: **THE LOOP** (the chosen boss as the hero via `animatedBossSprite` + "YOU ARE THE DUNGEON" + a 3-phase NIGHT→DAY→GROW loop using **real LOOPING-IDLE sprites** only (like the boss) — minion `animatedMinion('goblin1')` + boss-mini `animatedBossSprite`, both loaded with the run; the DAY adventurer's base sheet is on-demand so it starts as a ⚔ glyph and `_fillAdventurer` loads `adv-knight-v01` via `ensureAdventurerBaseSheet` and swaps the real `animatedAdventurer('knight')` in — no procedural `pixelSprite` fallbacks, no static frames. `inGameSnapshot` got a generic `_animatedFromAnim(animKey,size,{className,cacheKey})` [+ `_idleUnionCrop`] that `animatedBossSprite`/`animatedMinion`/`animatedAdventurer` all share — they loop the `<key>-idle-down` Phaser anim on a canvas), **CONTROLS** (live keybinds from the rebindable store via `getBind`/`keyLabel` + camera + gamepad note), **DARK PACTS** (sigil + the devil's-bargain explainer + the 6 rarity chips). Footer = progress dots + BACK/NEXT (last = "ENTER THE DUNGEON") + the tutorial-hints opt-in (persists `meta.tutorialEnabled` + `qf.gameplay.tutorials`). SKIP button + Esc both skip. Still gated on `meta.introSeen`; dev test-stage still skips it. Finish emits `INTRO_DISMISSED`.
- **Acceptance:**
...
### P4-3 — Misc hygiene `[S]` ✅ *(2026-06-19)*
...
- **Acceptance:**
  ...
  - [x] `PactDetailPopup` **and** `InspectPopup` (the canonical hover inspector — same `body`+`fixed` pattern) now honor the HUD stage scale. Fix = **translate-free anchoring**: position the outer element by measured `left`/`top` and apply `transform: scale(effectiveUiScale())` around a clean `transform-origin: 0 0` (the earlier `translate(%)`+`scale()`+non-center-origin combo offset the anchor — abandoned). `left`/`top` are computed from the *scaled* footprint (`offsetW/H × s`) so the anchor lands exactly: Pact `'left'` → right edge at x, v-center at y; `'below'` → h-center at x, top at y; Inspect's cursor-flip/clamp uses the scaled box too. The shared `tooltip-in` entrance keyframe was made scale-relative via `--tt-scale` (set per-popup) so the entrance lands on `scale(s)` instead of popping from `scale(1)`. *CDP-verified at scale 1 & 1.4: Pact `'left'` right=600/cy=300, `'below'` cx=600/top=300, footprint ×1.4 (280→392w); Inspect footprint ×1.4 and bottom-clamp lands at H−8 using the scaled height; keyframe `from` = `scale(0.96·s)`.*
- **Files:** `src/systems/EventBus.js`, `LeaderboardOverlay.js`, `ArchetypeSelectOverlay.js`, `BottomBar.js`, `TopBar.js`, `DungeonMechanicSystem.js`, `LongGameOverlay.js`, `src/hud/PactDetailPopup.js`, `src/hud/InspectPopup.js`, `src/hud/styles.css`.
