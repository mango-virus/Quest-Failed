...
export class AchievementsOverlay {
  ...
  _podiumStatsBlock(player, rank) {
    ...
    // registry, currently 7 — lilith, malakor, safira, zulgath,
    // nocturna, luna, rattlebones) rather than starters + achievement-
    // unlockable. This matches what the player sees on the recruit
    // screen, where all companions are visible (locked ones without
    // unlock conditions still appear as silhouette teasers). Pre-fix
    // the denominator was `3 + 1 = 4` which read as a bug since the
    // recruit screen shows more companions than that. As Nocturna /
    // Luna / Rattle Bones get their unlock conditions wired up, the
    // max reachable numerator grows toward 7 organically.
