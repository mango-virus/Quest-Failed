...
// 1.2.0 (2026-06-24): room connections moved from wall-to-wall to the one-tile-gap
...
// Unlike the 1.1.0->1.2.0 bump, 1.2.0 saves were carried forward (corridor stripped).
// 1.4.0 (2026-06-26): rooms now connect only at aligned wall MIDPOINTS (D3 locked
// design decision). Pre-midpoint layouts (1.1.0, 1.2.0, 1.3.0) have off-center
// connections inconsistent with the new rule and are ALL discarded on load — no
// forward migration. Any version !== '1.4.0' returns null from _migrate.
...
function _migrate(state) {
  ...
  // 1.4.0 introduced the wall-MIDPOINT connection rule (D3). All earlier saves
  // (1.1.0, 1.2.0, 1.3.0) have off-center connections incompatible with the new
  // rule and are discarded — no forward migration for any pre-1.4.0 version.
