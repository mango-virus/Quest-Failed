...
### NpcDirector (`src/systems/NpcDirector.js`)
The reactive brain behind the companion NPC, **Lilith**. Subscribes to ~30 EventBus events (player build actions, kills, flees, boss fights, level-ups, dungeon events, archetype hooks, etc.), picks a line + expression from `src/data/npcLines.json`, and emits `NPC_SAY` for the DOM `NpcCompanion` to render. Owns all pacing: priority bands (0 ambient → 3 major → 4 tutorial), per-category cooldowns, an interrupt rule + a small pending queue, a recently-said dedup ring, and a 23s idle timer that fills silence with context-aware ambient lines. Also reroutes the tutorial pipeline — intercepts `SHOW_TUTORIAL`, converts it to paged dialogue Lilith delivers herself, and fires the tutorial's `onClose` once the player pages past the last panel. Constructed in `Game.create()` *before* `TutorialSystem` (so its `INTRO_DISMISSED` handler registers first); destroyed in `Game.shutdown()`. Honours the `qf.gameplay.companion` setting (`off` → fully silent, tutorials fall back to `TutorialOverlay`; `quiet` → priority-2+ only; `normal` → full). Its renderer is `src/hud/NpcCompanion.js` (bottom-left peeking portrait + RPG chat bubble, 42 expression sprites cross-faded from `assets/npc/`); the two communicate via `NPC_SAY` / `NPC_POKE` / `NPC_TUTORIAL_DONE` / `NPC_CHOICE`.

She also: (1) **delivers the new-run intro** — `WelcomeIntroOverlay` emits `NPC_DELIVER_INTRO` (instead of its modal) when the companion is enabled; the Director plays the paged `intro` block from `npcLines.json`, the last page a Yes/No tutorial-hints choice that resolves to `INTRO_DISMISSED`. (2) **docks beside HUD menus** — the generic `Overlay` shell emits `HUD_MENU_OPENED/CLOSED { kind }` when given an `npcKind` opt (tagged on the 8 info overlays); the companion CSS-animates from her corner peek to a full-body stance left of the modal (`data-dock="menu"`, z-index lifted above the overlay backdrop) and the Director feeds `menu_*` commentary. (3) **is the Dark Pact broker** — `PactPicker.js` (the Grimoire) dropped its old demon-broker sprite roster; its contextual pact dialogue now routes to Lilith via `NPC_BROKER_SAY` and she docks beside the picker.

`npcLines.json` carries a keyed `specifics` bank (`specifics[domain][id]` for boss / advClass / event / room / minion / trap / item) — the Director prefers a bespoke line for the specific entity and falls back to the generic category. It also runs a build-suggestion advisor (the `suggestion` bank, each line gated on a game-state condition via `_checkCond`, surfaced during night-phase idle) and escalating inactivity nudges (`idle_nudge`, fired when the player goes a long while without acting). The bank is ~920 lines and exercises all 42 expression sprites.
